...
class ALeCaR5(page_replacement_algorithm):
...
#         return np.array([self.pollution_dat_x,self.pollution_dat_y ]).T
    ...
    def pageHitUpdate(self, page):
        ...
        val= np.random.rand()
        # heapq.heappush(self.PQ, (self.freq[page],val,page))
        self.npq.add(page)
    ...
    def getHeapMin(self):
        ...
        # while self.PQ[0][2] not in self.CacheRecency or self.freq[self.PQ[0][2]] != self.PQ[0][0] :
        #         heapq.heappop(self.PQ)
        # return self.PQ[0][2]
    ...
    def request(self,page) :
        ...
        if len(self.PQ) > 2*self.N:
            newpq = []
            for pg in self.CacheRecency:
                # newpq.append((self.freq[pg], np.random.rand(),pg))
