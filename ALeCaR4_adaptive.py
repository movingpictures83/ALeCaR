...
class ALeCaR4(page_replacement_algorithm):
    def __init__(self, cache_size, learning_rate=0, initial_weight=0.5, discount_rate=1, visualize=0):
        ...
        self.SampleHR =[]
        self.SAMPLE_W_0_SIZE =  10 * self.seq_len
        ...
        self.SampleChangeInHR_SIZE = 20 * self.seq_len
    ...
    def updateInRandomDirection(self) :
        ...
        if self.learning_rate  >= 0.99 :
            ...
            # if  val <=0.5:
                ...
                # self.learning_rate = min (  abs( self.learning_rate * 1.5 ) ,1)
           
            else:
                # self.learning_rate =max(   abs(self.learning_rate * 0.75 )  , 0.001)
    ...
    def updateLearningRates(self):
        ...
        if self.time % (self.seq_len) == 0:
            ...
            #         self.hitrate_negative_counter = 0
                ...
                sample_HR = round(self.SampleCacheHit   , 3) 

                # if sample_HR <= 0.001:
                #     self.learning_rate = self.reset_point
                    # self.W = np.array([self.initial_weight,1-self.initial_weight], dtype=np.float32)

                if (self.NewHR <= 0 and self.NewChangeInHR <= 0)  :
                ...
                if self.hitrate_zero_counter >= 20 :
                ...
                # if  self.NewChangeInHR < 0:
                    if self.hitrate_negative_counter >= 20 :
                        self.learning_rate = self.reset_point  
                        self.hitrate_negative_counter = 0
                    else :
