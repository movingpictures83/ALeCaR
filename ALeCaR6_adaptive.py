...
class ALeCaR6(page_replacement_algorithm):
...
#         return np.array([self.pollution_dat_x,self.pollution_dat_y ]).T
    ...
    def pageHitUpdate(self, page):
        ...
        self.accesstime[page]= -self.time
        heapq.heappush(self.PQ, (self.freq[page], self.accesstime[page], np.random.rand(), page))
        self.npq.add(page)
    ...
    def getHeapMin(self):
        ...
        # self.show_tree(self.PQ)
        # while self.PQ and (self.PQ[0][1] not in self.CacheRecency or self.freq[self.PQ[0][1]] != self.PQ[0][0]) :
        #     heapq.heappop(self.PQ)
        #     # print("Heap elem", self.PQ[0][2], "freq", self.PQ[0][0], "freq table", self.freq[self.PQ[0][2]])
        # return self.PQ[0][1]

        # print( heapq.heappop(self.PQ))

        while self.PQ[0][3] not in self.CacheRecency or self.freq[self.PQ[0][3]] != self.PQ[0][0] :
                heapq.heappop(self.PQ)
        return self.PQ[0][3]
       

        # return self.npq.popmin()
    ...
    def evictPage(self, pg):
        if pg in self.CacheRecency:
            self.CacheRecency.delete(pg)
            del self.accesstime[pg]
    ...
    def request(self,page) :
        ...
        if len(self.PQ) > 2*self.N:
            newpq = []
            for pg in self.CacheRecency:
                newpq.append((self.freq[pg], self.accesstime[pg], np.random.rand(), pg))
