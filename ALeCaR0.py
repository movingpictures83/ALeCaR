...
class ALeCaR0(page_replacement_algorithm):
...
#         return np.array([self.pollution_dat_x,self.pollution_dat_y ]).T
    ...
    def updateLearningRates(self,seq_len):
        if self.time % (seq_len) == 0:
            ...
            delta_LR = round(self.NewLR-self.PreviousLR,5)
            if (delta_LR) >= 0.00001 :
            ...
            if delta>=0:
                ...
                self.learning_rate =max(  self.learning_rate - abs(float( self.learning_rate) /2.0 ), 0.001)
