...
class ALeCaR7(page_replacement_algorithm):
    ...
    def updateLearningRates(self):
        ...
        if self.time % (self.seq_len) == 0:
            ...
            if delta>0  :
                ...
                # if self.SampleHitQ.full() and (sample_HR <= 0 ):
                    
                    self.learning_rate = self.reset_point
                    
                    if self.W[0] >0.99999 or self.W[0]<0.00001:
                       self.W = np.array([self.initial_weight,1-self.initial_weight], dtype=np.float32)

                # if  (self.NewHR *  self.W[0] <= 10^(-5) or self.NewHR * self.W[1]<= 10^(-5)):
                #     self.learning_rate = self.reset_point
                    # if self.W[0] >0.99 or self.W[0]<0.01:
                #     self.W = np.array([self.initial_weight,1-self.initial_weight], dtype=np.float32)
                ...
                if self.Sample_W_0 >= 0.95 * self.SAMPLE_W_0_SIZE  :
                    self.W = np.array([self.initial_weight,1-self.initial_weight], dtype=np.float32)
                    # self.lru_flag= True
